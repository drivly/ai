...
describe('ApiClient', () => {
  ...
  describe('constructor', () => {
    ...
    it('should use default baseUrl for requests if not provided', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/test', expect.any(Object))
    })
  ...
  describe('utility methods', () => {
    it('should call get with correct path for list method', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/api/functions?limit=10&page=1', expect.any(Object))
    ...
    it('should call get with correct path for getById method', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/api/functions/123', expect.any(Object))
    })
    ...
    it('should call post with correct path for create method', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/api/functions', {
        method: 'POST',
    ...
    it('should call patch with correct path for update method', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/api/functions/123', {
        method: 'PATCH',
        headers: expect.any(Object),
        body: JSON.stringify(data),
      })
    ...
    it('should call get with correct path and query for search method', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/api/functions/search?limit=10&q=test+query', expect.any(Object))
    })
    ...
    it('should handle where parameter correctly in query params', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://apis.do/test?where=%7B%22name%22%3A%7B%22equals%22%3A%22Test%22%7D%7D', expect.any(Object))
    })
  })
})
