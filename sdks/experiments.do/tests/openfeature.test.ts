...
describe('VercelFlagsProvider', () => {
  ...
  it('should resolve boolean evaluations', async () => {
    const mockEvaluateFlag = vi.spyOn(provider, 'evaluateFlag')
      .mockResolvedValue({ variant: 'test-variant', value: true })
  ...
  it('should resolve string evaluations', async () => {
    const mockEvaluateFlag = vi.spyOn(provider, 'evaluateFlag')
      .mockResolvedValue({ variant: 'test-variant', value: 'test-value' })
  ...
  it('should resolve number evaluations', async () => {
    const mockEvaluateFlag = vi.spyOn(provider, 'evaluateFlag')
      .mockResolvedValue({ variant: 'test-variant', value: 42 })
  ...
  it('should resolve object evaluations', async () => {
    const mockEvaluateFlag = vi.spyOn(provider, 'evaluateFlag')
      .mockResolvedValue({ variant: 'test-variant', value: { color: 'blue' } })
...
describe('ExperimentsClient', () => {
  ...
  it('should use OpenFeature provider for variant resolution', async () => {
    const mockResolveObjectEvaluation = vi.spyOn(VercelFlagsProvider.prototype, 'resolveObjectEvaluation')
      .mockResolvedValue({
        value: { color: 'blue' },
        variant: 'test-variant'
      })
    ...
    expect(mockResolveObjectEvaluation).toHaveBeenCalledWith(
      'test-experiment',
      {},
      { userId: 'user-123' }
    )
  ...
  it('should track metrics with the track method', async () => {
    const mockRecordMetric = vi.spyOn(VercelFlagsProvider.prototype, 'recordMetric')
      .mockResolvedValue({})
    const mockCreate = vi.spyOn(client['api'], 'create')
      .mockResolvedValue({})
  ...
  it('should respect analyticsEnabled option', async () => {
    ...
    const mockCreate = vi.spyOn(clientWithoutAnalytics['api'], 'create')
      .mockResolvedValue({})
    
    await clientWithoutAnalytics.trackEvent(
      'test-experiment',
      'test-variant',
      'button_click'
    )
