...
export class MDXCollectionHandler<T = Record<string, unknown>> implements CollectionMethods<T> {
  ...
  async find(options: QueryOptions = {}): Promise<ListResponse<T>> {
    try {
      ...
      const documents = await Promise.all(
        ids.map(id => this.fsHandler.readFile<T>(this.collection, id))
      )
      ...
      const { paginatedDocs, totalDocs, totalPages } = paginateDocuments(
        sortedDocs,
        options.limit,
        options.page
      )
      ...
      throw new MDXDBError(
        `find operation failed on collection '${this.collection}': ${(error as Error).message}`,
        500,
        'find',
        this.collection
      )
  ...
  async create(data: Partial<T>): Promise<T> {
    try {
      ...
      await this.fsHandler.writeFile(
        this.collection,
        id,
        dataWithoutContent,
        content
      ...
      throw new MDXDBError(
        `create operation failed on collection '${this.collection}': ${(error as Error).message}`,
        500,
        'create',
        this.collection
      )
  ...
  async update(id: string, data: Partial<T>): Promise<T> {
    try {
      ...
      const content = (data as any).content !== undefined
        ? (data as any).content
        : existingDoc.content
  ...
  async search(query: string, options: QueryOptions = {}): Promise<ListResponse<T>> {
    try {
      ...
      throw new MDXDBError(
        `search operation failed on collection '${this.collection}': ${(error as Error).message}`,
        500,
        'search',
        this.collection
      )
