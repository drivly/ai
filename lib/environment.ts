export function validateEnvironment() {
  ...
  const missing = requiredVars
    ...
    .map(v => ({ ...v, value: process.env[v.name] }));
    
  if (missing.length > 0) {
    const criticalMissing = missing.filter(v => v.critical);
    if (criticalMissing.length > 0) {
      console.error('Critical environment variables missing:', criticalMissing.map(v => v.name));
      throw new Error('Missing critical environment variables');
    } else {
      console.warn('Non-critical environment variables missing:', missing.map(v => v.name));
    }
  }
