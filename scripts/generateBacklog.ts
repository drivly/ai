...
async function updateRoadmapComment(markdown: string): Promise<void> {
  try {
    ...
    const existingComment = comments.find((comment: any) => 
      comment.body.includes('## ROADMAP STATUS UPDATE')
    )
...
async function createIssuesFromComment(): Promise<void> {
  try {
    ...
    const roadmapComment = comments.find((comment: any) => 
      comment.body.includes('## ROADMAP STATUS UPDATE')
    )
    ...
    const newIssueLines = roadmapComment.body
      .split('\n')
    ...
    for (const line of newIssueLines) {
      ...
      const labelMatch = Object.entries(labelToCategoryMap).find(([label, cat]) => 
      ...
      const updatedBody = roadmapComment.body.replace(
        line,
        `✅ CREATED: ${result.trim()} (was: ${line})`
      )
