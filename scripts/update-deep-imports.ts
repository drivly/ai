...
const EXCLUDE_PATTERNS = ['node_modules', 'workers', 'pkgs', 'websites', 'templates']
  ...
  try {
    ...
    while ((match = staticImportRegex.exec(content)) !== null) {
      ...
      if (!absolutePath.startsWith('.')) {
        updatedContent = updatedContent.replace(new RegExp(`from\\s+['"]${fullImportPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`, 'g'), `from '@/${absolutePath}'`)
      }
    }
