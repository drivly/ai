...
The [`Site`](/docs/sdks/sites.do#Site) function in [`sites.do`](/docs/sdks/sites.do) allows a [Website](/docs/website) to be defined via [Next.js](https://nextjs.org) for either MongoDB, PostgreSQL, or SQLite:

```typescript
import { Site } from 'sites.do'
...
The `site` object provides access to the website:

```typescript
import { site } from 'sites.do'

const home = await site.pages.home
```