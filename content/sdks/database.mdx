...
const db = DB({
  ...
  collectionName1: { /* schema */ },
  collectionName2: { /* schema */ },
})
```

### Collection Methods

#### db.{collectionName}.find(options?)
...
#### db.{collectionName}.update(id, data)
...
#### db.{collectionName}.search(query, options?)
