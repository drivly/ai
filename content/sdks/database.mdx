...
const db = DB({
  ...
  resourceType1: { /* schema */ },
  resourceType2: { /* schema */ },
})
```

### Resource Methods

#### db.{resourceType}.find(options?)

Retrieves multiple documents from a resource with optional filtering, sorting, and pagination.
...
#### db.{resourceType}.create(data)

Creates a new document in the resource.
...
#### db.{resourceType}.update(id, data)
...
#### db.{resourceType}.search(query, options?)
...
const db = DB({
  products: {
    ...
    category: 'categories', // Reference to another resource type
  },
  categories: {
    name: 'text',
