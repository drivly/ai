...
const db = DB({
  ...
  collectionType1: { /* schema */ },
  collectionType2: { /* schema */ },
})
```

### Collection Methods

#### db.{collectionType}.find(options?)
...
#### db.{collectionType}.update(id, data)
...
#### db.{collectionType}.search(query, options?)
