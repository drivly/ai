...
const blogPost = await ai.writeBlogPost({ topic: 'The Future of AI', tone: 'Optimistic' }, {}, { model: 'anthropic/claude-3.7-sonnet:creative' })

// Use a model with code capabilities for programming tasks
const code = await ai.generateCode({ language: 'TypeScript', task: 'Create a React component for a form' }, {}, { model: 'openai/gpt-4-turbo:code' })
```
