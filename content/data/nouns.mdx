...
In our semantic data model, Nouns define the types or categories that your business entities fall into (like Customer, Product, Order, User, Document). They serve as templates or schemas for creating specific instances (Things).
...
```
[Noun Type] → defines → [Thing (Instance)]
```

For example, the Noun "Customer" defines what properties and relationships a specific customer Thing can have.

```
Customer (Noun) → defines → ACME Corp (Thing)
```

When used in Actions (Subject-Predicate-Object triples), Things that are instances of Nouns can be either:
- The Subject performing an action
- The Object receiving an action
