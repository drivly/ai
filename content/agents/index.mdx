...
- **Tool Usage**: Agents can use Functions, Workflows, Searches, and Integrations as tools

## Using Tools with Agents

A powerful capability of Agents.do is the ability to use various platform components as tools. This creates a composable architecture where agents can leverage specialized capabilities to accomplish complex tasks.
...
Agents can use entire workflows as tools, enabling them to trigger complex business processes:
...
Agents can use searches to retrieve relevant information from various data sources:

```typescript
// Example of an agent using searches as tools
const knowledgeAgent = Agent({
  name: 'KnowledgeAssistant',
  tools: [
    // Searches available as tools
    searches.productCatalog,
    searches.customerRecords,
    searches.knowledgeBase,
    searches.previousConversations
  ]
})

// The agent can use searches to gather context
const response = await knowledgeAgent.handle({
  question: "What's our return policy for electronics?",
  context: { customerId: 'cus_12345' }
})
```

### Integrations and Actions as Tools

Agents can use integrations and actions to interact with external systems and perform operations:

```typescript
// Example of an agent using integrations and actions as tools
const operationsAgent = Agent({
  ...
  tools: [
    ...
    // Actions available as tools
    actions.createTicket,
    actions.updateCustomerRecord,
    actions.sendNotification
  ]
})

// The agent can use integrations and actions to perform tasks
...
Importantly, the same tools that are available to AI Agents can also be accessed by human users through interfaces like:
