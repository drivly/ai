...
try {
  ...
  const files = fs.readdirSync(SOURCE_DIR).filter((file) => file.endsWith('.jsonld'))
  ...
  fs.writeFileSync(path.join(BUILD_DIR, 'index.ts'), `// Generated exports for JSON-LD contexts\n${exportStatements}\n`)
