...
describe('DurableObjectsNoSQL', () => {
  ...
  it('should initialize the database schema on construction', () => {
    ...
    expect(mockStorage.sql.exec).toHaveBeenCalledWith(
      expect.stringContaining('CREATE INDEX IF NOT EXISTS idx_collection')
    );
  });
...
describe('Collection', () => {
  ...
  describe('insertOne', () => {
    it('should insert a document and return an id', async () => {
      ...
      expect(mockStorage.sql.exec).toHaveBeenCalledWith(
        ...
        'test-uuid',
        'test',
        expect.stringContaining('New Document')
      );
  ...
  describe('insertMany', () => {
    it('should insert multiple documents and return ids', async () => {
      ...
      expect(mockTransaction.put).toHaveBeenCalledTimes(2);
