      ...
      if (item) {
        ...
        if (searchTerm) {
          try {
            ...
            item.children = documents.map(doc => new DocumentItem(
  ...
  async getDocumentsForCollection(collection: CollectionItem): Promise<DocumentItem[]> {
    try {
      if (collection.children.length === 0) {
        ...
        collection.children = documents.map(doc => new DocumentItem(
          doc.name || doc.id || 'Unknown',
          doc.id || 'Unknown',
  ...
  async loadCollections() {
    try {
      ...
      this.collections = collections.map(collection => 
        new CollectionItem(collection, collection)
      );
...
async function openDocumentInEditor(item: DocumentItem): Promise<void> {
  try {
    ...
    const tempFile = path.join(extensionContext.storageUri.fsPath, `${item.collectionName}-${item.id}.yaml`);
