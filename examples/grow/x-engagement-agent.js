...
export default class XEngagementAgent {
  ...
  async initialize() {
    ...
    this.agent = new Agent({
      ...
      instructions:
        'You are a social media engagement specialist focused on engaging with relevant content on X (Twitter). Your goal is to increase brand visibility, build industry authority, and generate high-quality leads.',
      model: openai('gpt-4o'),
      tools: tools,
    })
  ...
  shouldEngageWithTweet(tweet, criteria = this.engagementCriteria) {
    ...
    const hasEnoughEngagement = (tweet.public_metrics.like_count || 0) >= criteria.min_likes || (tweet.public_metrics.retweet_count || 0) >= criteria.min_retweets
    ...
    const containsKeyword = criteria.keywords.some((keyword) => tweetTextLower.includes(keyword.toLowerCase()))

    if (!containsKeyword) {
      return false
    }

    const containsExcludedKeyword = criteria.exclude_keywords.some((keyword) => tweetTextLower.includes(keyword.toLowerCase()))
