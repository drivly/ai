...
chromaticTest('admin login page', async ({ page }, testInfo) => {
  ...
  try {
    ...
    if ((await errorElement.count()) > 0) {
      ...
      try {
        await expect(page.locator('input[type="email"]').first()).toBeVisible({ timeout: visibilityTimeout / 2 })
        await expect(page.locator('input[type="password"]').first()).toBeVisible({ timeout: visibilityTimeout / 2 })
        await expect(page.locator('button[type="submit"]').first()).toBeVisible({ timeout: visibilityTimeout / 2 })
      } catch (error) {
        console.log('Could not find login form elements, continuing with screenshot anyway')
      }
    }
  } catch (error) {
