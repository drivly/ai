...
app.get('/', async (c) => {
  ...
  try {
    ...
    const exports = (await moduleExports.json()) as Record<
      string,
      {
        type: string
...
app.post('/deploy/agent/:id', async (c) => {
  ...
  try {
    ...
    const deployResponse = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${c.env.CF_ACCOUNT_ID}/workers/dispatch/namespaces/${c.env.CF_NAMESPACE_ID}/scripts/${agentData.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
      {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${c.env.CF_API_TOKEN}`,
...
app.all('*', async (c) => {
  ...
  try {
    const scriptResponse = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${c.env.CF_ACCOUNT_ID}/workers/dispatch/namespaces/${c.env.CF_NAMESPACE_ID}/scripts/${subdomain}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${c.env.CF_API_TOKEN}`,
