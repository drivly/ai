...
/**
 * Search Things collection using vector search
 * @param query Search query text
 * @param limit Maximum number of results to return
 * @returns Array of matching Thing documents
 */
export async function searchThings(query: string, limit: number = 10): Promise<any[]> {
  try {
 ...
 * @returns Array of matching Thing documents
 */
export async function hybridSearchThings(query: string, limit: number = 10): Promise<any[]> {
  try {
    const queryEmbedding = await generateEmbedding(query)
    ...
    console.error('Error performing hybrid search on Things:', error)
    throw error
  }
}
