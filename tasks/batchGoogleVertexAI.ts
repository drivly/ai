...
export const processBatchGoogleVertexAI = async ({ input, req, payload }: any) => {
  ...
  try {
    ...
    if (checkStatus && batch.providerBatchId) {
      const response = await fetch(`https://us-central1-aiplatform.googleapis.com/v1/projects/${process.env.GOOGLE_PROJECT_ID}/locations/us-central1/batchPredictionJobs/${batch.providerBatchId}`, {
        method: 'GET',
        headers: {
      ...
      const status = batchStatus.state === 'JOB_STATE_SUCCEEDED' ? 'completed' : 
                    batchStatus.state === 'JOB_STATE_FAILED' ? 'failed' :
                    'processing'
