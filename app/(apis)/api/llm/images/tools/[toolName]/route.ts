...
const getToolIcon = async (toolName: string): Promise<string> => {
  const internalTools = ['fetchWebsiteContents', 'worker', 'testTool']

  if (internalTools.includes(toolName)) {
    return 'https://workflows.do/apple-icon.png'
  }

  const toolMetadata = await fetch(`http://prodthermospubliclb-2130399048.us-east-1.elb.amazonaws.com/api/actions/${toolName}`, {
    headers: {
