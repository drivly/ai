...
export const GET = API(async (req, { db, user, payload, params }) => {
  ...
  // Schema.org Nouns
  const graph = await fetch('https://schema.org/version/latest/schemaorg-current-https.jsonld').then(res => res.json()).then(data => data['@graph'])
  const schemaResults = await payload.db.connection.collection('nouns').bulkWrite(graph.map((thing: any) => {
  ...
  const apps = await fetch('https://backend.composio.dev/api/v1/apps', composio).then(res => res.json()).then(data => data.items)
  const categories = await fetch('https://backend.composio.dev/api/v1/apps/list/categories', composio).then(res => res.json()).then(data => data.items)
  const triggers = await fetch('https://backend.composio.dev/api/v1/triggers', composio).then(res => res.json())
  const actions = await fetch('https://backend.composio.dev/api/v1/actions', composio).then(res => res.json()).then(data => data.items)
  const integrations = await payload.db.connection.collection('integrations').bulkWrite(apps.map((app: any) => {
